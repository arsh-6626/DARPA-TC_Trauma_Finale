...
- wound: visible injuries (bleeding, deformities, blood soaked clothing, gunshot wounds) or any Burn injuries or casualty holding wounded area by hand.
...
"""
        ...
        self.report_verify_amputation = """Verify with respect to the video, if it shows severance of limbs or not? Classify severance as 'Amputation' and then return the correct report in the same format only, as a dictionary given below.
{head:[<Class>],
torso:[<Class>],
upper extremity:[<Class>],
lower extremity:[<Class>]}""" 
...
If multiple casualties are visible, focus on **the largest and most visible casualty**.Ignore camouflage clothing, nets, helmets, bags, bottles, white-black boxes, etc.
...
- Pool of blood around the casualty ?
...
Return **only your descriptive narrative** inside the `<answer></answer>` tag."""
        ...
        self.sevhem_ma_report = """Using the description above and the video, analyze the casualty (actors and mannequins in the video are all casualties). If multiple casualties are visible, focus on the largest and most visible one. Prioritize the casualty over environmental factors.
...
Evaluate the casualty in the following **assessment categories** and classify them under the given criteria:
...
"""
    ...
    def trauma_report(self):
        prompt= self.trauma_report_prompt
        return prompt
    
    def trauma_regen(self):
...
class configs:
    ...
    def trauma_desc(self):
        return self.trauma_desc_config
    def trauma_desc_regen(self):
        return self.trauma_desc_regen_config
    def sh_desc(self):
        return self.sh_desc_config
    def sh_desc_regen(self):
        return self.sh_desc_regen_config
    def standard_report(self):
        return self.standard_report_config